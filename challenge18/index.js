(function () {
    ...
    */
    ...
    //1- Opção: cleanCPF.replace( /(\d{3}) (\d{2}) (\d{3})(\d\d)/g,'$1.$2.$3-$4');

    console.log
        (cleanCPF.replace(/(\d{3}) (\d{2}) (\d{3})(\d{2})/g, function (regex, agr1, agr2, agr3, agr4) {
            return agr1 + '.' + agr2 + '.' + agr3 + '-' + agr4;
        }));
    ...
    */
    ...
    console.log('Os meses de janeiro, junho e julho começam com a letra'.match(/ju(n|l)ho/g));
    ...
    */
    ...
    // console.log("<div><section><blockquote>Texto <img /></blockquote></section></div>".match(/<div>|<section>|<blockquote>/g))
    ...
    */
    ...
    console.log('<h1>Título da página</h1><p>Este é um parágrafo</p><footer>Rodapé</footer>'.replace(/<(\w+)>([^<]+)<\/\w+/g,
        '<$1> O texto dentro da tag "S1" é "$2"</$1>\n'));
})();